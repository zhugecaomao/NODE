...
P_Files_Scan()
{
	...
	if !hFFF_Callback
	{
		...
		if A_IsUnicode													; install library according to AHK version (ANSI/Unicode)
			FileInstall, Resources\fffW.dll, %A_ScriptDir%\res\bin\fff.dll, 1
		else
			FileInstall, Resources\fffA.dll, %A_ScriptDir%\res\bin\fff.dll, 1
		hFFF_Callback := RegisterCallback("P_Files_Int_Callback", "CDecl F", 5, "")	; register callback for the search function
		hFFF_Lib := DllCall("LoadLibrary" AW, "Str", A_ScriptDir "\res\bin\fff.dll")	; load search library
		hFFF := DllCall("GetProcAddress", Ptr, hFFF_Lib, AStr, "FileFindFilter")	; get procedure address for faster repetitive operation
	...
	TypeList := RegExReplace(TypeList, "sU)(^\||\|$)")	; trim colon from either end of the string
	...
	Loop, Parse, FolderList, |	; feed paths one at a time to the search function (speed loss in favor of separate DBs, one per path)
	{
		DllCall(hFFF, "Str", A_LoopField, "Str", TypeList, "Str", "*", "UInt", 1, Ptr, hFFF_Callback, "CDecl UInt")	; call the search library
		NewFileData := P_Files_Int_Callback(0, 1, 0, 0, 0)	; call the callback with the 'dump data' switch (first param 0, second param 1)
		StringTrimRight, NewFileData, NewFileData, 1		; remove trailing NewLine to avoid DB item miscount
	...
	if !A_IsCompiled
	{
		...
		SetTimer, P_Files_ttoff, -3500					; dismiss tolltip after 3.5 seconds
	}
	return
...
P_Files_ConfGUI(GUI_ID)
{
	...
	, P_Files_C_Edit2, P_Files_C_Remove2
	RegExMatch(A_ThisFunc, "U)_(?<PlugName>.*)_", O)
	...
	Gui, Add, Edit, x16 y423 w232 h20 -Multi gP_Files_Buttons vP_Files_C_LastPath, <type a path to add to the list>
	...
	Gui, Add, Edit, x350 y423 w73 h20 -Multi gP_Files_Buttons vP_Files_C_LastExt, <extension>
...
P_Files_ConfSave()
{
	...
	StringTrimRight, P_Files_C_UserFolderList, P_Files_C_UserFolderList, 1		; see, I'm stripping trailing colon !
	StringTrimRight, P_Files_C_UserTypeList, P_Files_C_UserTypeList, 1			; see, I'm stripping trailing colon here too !
...
P_Files_Actions(Line)
{
	...
	/*
	;--start of dynamic action
	;action analyzed and added to ActionList if conditions met
	Path := ReadTag(Line,"Path")		;reads path tag from Object's DB line
	SplitPath, Path, ,, FExt
	IfEqual, FExt, lnk
		ActionList = %ActionList%`r`n<Action=%OPlugName%.Create Alias><Scan1=Create Alias>
	;--end of action
	*/
...
P_Files_Execute(Line, Action, SrchStrng)
{
	...
	IfEqual, Action, %OPlugName%.Edit Text
	{
		...
		IniRead, Editor, %File_conf%, %OPlugName%, Editor, %A_Space%
...
;		EditorSyntax := P_Files_Int_Storage("Editor", 2)	; we may add this one day so let it be a reminder
	...
	IfEqual, Action, %OPlugName%.Create Alias
	{
		...
		{
			DllCall(P_Files_Int_Storage("FFFh"), "Str", A_ScriptDir "\alias", "Str", "lnk", "Str", "*", "UInt", 1, Ptr, P_Files_Int_Storage("FFFc"), "CDecl UInt")
			;P_Files1_DB := P_Files_Int_Callback(0, 1, 0, 0, 0)	; call the callback with the 'dump data' switch
			;StringTrimRight, P_Files1_DB, P_Files1_DB, 1		; remove trailing NewLine
			;FileAppend, %P_Files1_DB%, %A_ScriptDir%\db\Files1.ndb
			DBContent := P_Files_Int_Callback(0, 1, 0, 0, 0)	; call the callback with the 'dump data' switch
			...
			;NOTE : The below command directly changes the host's variable
			;something like this should be used only when fully understanding the results.
			;Its done here to avoid a restart of the app after addtion of an alias
			FileRead, P_Files1_DB, %A_ScriptDir%\db\Files1.ndb
		}
